...
const AuthModel = ({setShowAuthModel}) => {
  ...
  const handleEmailAuth = () => {
    if (isOtpVerification) {
      ...
      setScreenTransition(true)
      setOtpValues(['', '', '', '']) // Reset OTP values before verification
      setTimeout(() => {
  ...
  const handleResendOtp = (context) => {
    ...
    if (context === 'reset') {
      ...
      // API call would go here for password reset OTP
    } else if (context === 'verification') {
      // Account verification specific logic
      console.log('Resending account verification OTP')
      // API call would go here for verification OTP
    }
    ...
    <div className={`fixed top-0 left-0 w-full h-full px-4 z-50 flex items-center justify-center bg-[#000000a1] backdrop-blur-sm transition-all duration-300 ease-in-out ${isExiting ? 'opacity-0' : isEntering ? 'opacity-0' : 'opacity-100'}`}>
      ...
      >
        ...
        <div className="w-full md:w-3/5 p-8 md:px-16 bg-black text-white transition-all duration-300 ease-in-out">
          ...
          {isForgotPassword ? (
            <div className={`transition-all duration-300 ease-in-out ${screenTransition ? 'opacity-0 translate-y-10' : 'opacity-100 translate-y-0'}`}>
              <div className="flex items-center mb-8">
                <button
                  onClick={forgotPasswordStep === 1 ? backToLogin : () => {
                    setScreenTransition(true)
                    setTimeout(() => {
                  ...
                  className="text-[#cccccc] hover:text-white transition-colors duration-200 flex items-center mr-2 group"
                >
              ...
              <div className="flex items-center justify-center space-x-2 mb-8">
                {[1, 2, 3].map(step => (
                  ...
                  >
                    ...
                    {step < 3 && (
                      <div className={`w-12 h-0.5 ${forgotPasswordStep > step ? 'bg-white' : 'bg-[#444]'}`}></div>
                    )}
                  </div>
                ))}
              </div>
                ...
                <>
                  ...
                  <div className="w-full relative mb-6">
                    <input
                      ...
                      className={`w-full ${
                        emailError ? "bg-[#440b0b] border-red-500" : "bg-[#1f1f1f] border-[#3a3a3a]"
                  ...
                  <button
                    ...
                    className={`relative flex items-center justify-center bg-[#f2f2f2] text-black font-inter w-full font-medium py-3 px-4 rounded-md transition duration-200 
                      ${isVerifying ? 'opacity-70 cursor-not-allowed' : 'hover:bg-[#e1e1e1]'}`}
                  >
                    {isVerifying ? (
                      <>
              ...
              {forgotPasswordStep === 2 && (
                <>
                    ...
                    {otpValues.map((value, index) => (
                      <input
                        ...
                        className={`w-12 h-12 text-center text-lg font-medium rounded-md focus:outline-none transition-all duration-200
                          ${otpError ? 'bg-[#440b0b] border border-red-500' : 
                  ...
                  <div className="text-center mt-6">
                    <p className="text-[#cccccc] text-sm mb-2">Didn't receive the code?</p>
                    <button 
                      onClick={() => handleResendOtp('reset')}
                      disabled={resendCooldown}
                      className={`text-sm ${resendCooldown ? 'text-[#777777] cursor-not-allowed' : 'text-white cursor-pointer hover:underline'}`}
                    >
                      {resendCooldown ? `Resend code in ${cooldownTime}s` : 'Resend code'}
                    </button>
                  </div>
                </>
              )}
                ...
                <>
                  ...
                  <div className="w-full relative mb-5">
                    <input
                      ...
                      className={`w-full ${
                        passwordError ? "bg-[#440b0b] border-red-500" : "bg-[#1f1f1f] border-[#3a3a3a]"
                      } text-white font-inter text-base py-3 px-5 rounded-md border focus:outline-none focus:border-[#cccccc] transition-all duration-200`}
                    />
                    <button
                      type="button"
                  ...
                  <div className="w-full relative mb-2">
                    <input
                      ...
                      className={`w-full ${
                        !passwordsMatch ? "bg-[#440b0b] border-red-500" : "bg-[#1f1f1f] border-[#3a3a3a]"
                      } text-white font-inter text-base py-3 px-5 rounded-md border focus:outline-none focus:border-[#cccccc] transition-all duration-200`}
                    />
                    <button
                      type="button"
                  ...
                  <div className="text-[#aaaaaa] text-xs mb-6">
                    ...
                    <ul className="list-disc pl-5 space-y-0.5">
                      <li className={newPassword.length >= 6 ? "text-green-500" : ""}>Be at least 6 characters long</li>
                      <li className={/[A-Z]/.test(newPassword) ? "text-green-500" : ""}>Include at least one uppercase letter</li>
            ...
            <div className={`transition-all duration-300 ease-in-out ${screenTransition ? 'opacity-0 translate-x-10' : 'opacity-100 translate-x-0'}`}>
              <div className="flex items-center mb-8">
                <button
                ...
                <h1 className="font-inter font-semibold text-2xl">Verify your email</h1>
              </div>
              ...
              {verificationSuccess ? (
                <div className="flex flex-col items-center justify-center py-8 animate-fadeIn">
                  ...
                  <h2 className="text-2xl font-semibold mb-2">Successfully Verified!</h2>
                  <p className="text-[#cccccc] text-center">Your account has been created successfully.</p>
                </div>
              ) : (
                <>
                  ...
                  <button
                    ...
                    disabled={isVerifying}
                    className={`relative flex items-center justify-center mt-2 bg-[#f2f2f2] text-black font-inter w-full font-medium py-3 px-4 rounded-md transition duration-200 ${isVerifying ? 'opacity-70 cursor-not-allowed' : 'hover:bg-[#e1e1e1]'}`}
                  >
                    {isVerifying ? (
                      <>
                        <span className="absolute left-1/2 transform -translate-x-1/2">
                  ...
                  <div className="text-center mt-8">
                    ...
                    <button 
                      ...
                      disabled={resendCooldown}
                      className={`text-sm ${resendCooldown ? 'text-[#777777] cursor-not-allowed' : 'text-white hover:underline'}`}
                    >
                      {resendCooldown ? `Resend code in ${cooldownTime}s` : 'Resend code'}
                    </button>
                  </div>
